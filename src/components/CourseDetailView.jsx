...
export default function CourseDetailView() {
  ...
  if (!activeCourse) {
    return (
      <div className="flex flex-col h-screen bg-slate-50 items-center justify-center p-8 text-center">
        ...
        <button 
          onClick={() => navigate('/')}
          className="px-6 py-2.5 bg-indigo-600 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95"
        >
          Return Home
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-white font-sans text-slate-900 overflow-hidden">
      <Navbar onHome={() => navigate('/')} activeCourse={activeCourse} />
